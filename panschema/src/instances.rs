...
/// One record whose authoring fell short of a slot's `expand_against`
/// declaration; each kind carries its own story, and validation words
/// each in its own sentence.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub enum ExpansionGapKind {
    /// The record supplied no usable base: what it held instead. Its
    /// bare values were read as authored.
    UnusableBase(&'static str),
    /// The record authored one or more inline records at a slot whose
    /// declaration expects external references — one finding per
    /// claiming record and slot, whatever the authoring spelling.
    InlineRecord,
}

/// One record whose authoring fell short of a slot's `expand_against`
/// declaration — reported rather than silent.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
...
pub struct UnusableCollectionEntry {
    /// The record whose slot holds the entry; `None` when the collection
    /// sits on the dataset container (or a vessel root) itself.
...
impl InstanceSet {
    ...
    pub fn from_linkml_data(schema: &SchemaDefinition, data: &serde_norway::Value) -> Self {
        ...
        for (key, value) in container {
            ...
            let (class_targets, all_classes) = class_targets_of(schema, &ranges);
            ...
            let multivalued = crate::linkml_resolve::effective_cardinality(slot).multivalued;
            if is_record_collection(multivalued, &class_targets, all_classes) {
                ...
                let root_record = authored_root_id
                    .clone()
                    .unwrap_or_else(|| root_name.clone());
                let expanded_storage;
                let value: &serde_norway::Value = match loader.expand_root_class_value(
                    root_name,
                    slot_name,
                    container,
                    value,
...
/// The deduplicated class targets among `ranges` — repeated branches
/// naming one class are one candidate — and whether every range is a
/// class, computed one way at every depth.
fn class_targets_of<'r>(
    schema: &SchemaDefinition,
    ranges: &'r [String],
) -> (Vec<&'r String>, bool) {
    let all_classes = !ranges.is_empty() && ranges.iter().all(|r| schema.classes.contains_key(r));
    let mut targets: Vec<&String> = ranges
        .iter()
        .filter(|r| schema.classes.contains_key(*r))
        .collect();
    let mut seen = std::collections::BTreeSet::new();
    targets.retain(|c| seen.insert(c.as_str()));
    (targets, all_classes)
}

/// Whether a slot's value holds a collection of records: effectively
/// multivalued, ranging over one class or a union made only of classes.
/// A union mixing classes with types falls to record-level rules — a
/// scalar there could legitimately be a literal.
fn is_record_collection(multivalued: bool, class_targets: &[&String], all_classes: bool) -> bool {
    multivalued && (class_targets.len() == 1 || all_classes)
}
...
struct LinkmlLoader<'a> {
    ...
    /// map, and the full inheritance walk is too costly to repeat per
    /// record. `Rc` so a caller can hold the map across `&mut self` work.
    effective_slots_by_class: std::collections::BTreeMap<
        String,
        std::rc::Rc<std::collections::BTreeMap<String, crate::linkml_resolve::ResolvedSlot>>,
    >,
...
impl LinkmlLoader<'_> {
    /// `class_name`'s effective slots with provenance, resolved once and
    /// cached; the returned handle stays valid across further loading.
    fn resolved_slots(
        &mut self,
        class_name: &str,
    ) -> std::rc::Rc<std::collections::BTreeMap<String, crate::linkml_resolve::ResolvedSlot>> {
        if !self.effective_slots_by_class.contains_key(class_name) {
            let slots = self
                ...
                .get(class_name)
                .map(|class| {
                    crate::linkml_resolve::resolve_effective_slots_with_provenance(
                        class,
                        self.schema,
                    )
                })
            ...
            self.effective_slots_by_class
                .insert(class_name.to_string(), std::rc::Rc::new(slots));
        }
        std::rc::Rc::clone(&self.effective_slots_by_class[class_name])
    }
    ...
    fn simple_dict_slot(&mut self, class_name: &str) -> Option<String> {
        ...
        let mut open = slots
            .iter()
            .filter(|(_, rs)| {
                !rs.definition.identifier && !rs.definition.key && !rs.definition.designates_type
            })
            .map(|(name, _)| name);
    ...
    fn class_carries(&mut self, class_name: &str, slot_name: &str) -> bool {
        self.resolved_slots(class_name).contains_key(slot_name)
    }
    ...
    /// Root-container parity for a bound class-ranged slot: bare string
    /// entries expand (or gap) exactly as they would on a nested record,
    /// and inline mappings get the same per-record finding, before the
    /// collection machinery reads the value. `None` when nothing
    /// changed, so the caller keeps the authored value.
    fn expand_root_class_value(
        &mut self,
        root_class: &str,
        slot: &str,
        container: &serde_norway::Mapping,
        ...
        match value {
            ...
            serde_norway::Value::Sequence(items) => {
                ...
                for item in items {
                    match item {
                        serde_norway::Value::String(text) => {
                            match self.expand_bare(expand, slot, record, text, true) {
                                Some(expanded) => {
                                    changed = true;
                                    out.push(serde_norway::Value::String(expanded));
                                }
                ...
                changed.then_some(serde_norway::Value::Sequence(out))
            }
    ...
    /// Expand one bare value against the record's base, or record why it
    /// could not be — the one definition of "bare" (non-empty, carrying
    /// no `:`; an undeclared-prefix CURIE stays as authored and surfaces
    /// through the resolution checks) and of the gap. `require_external`
    /// is the class-ranged contract: an expansion that does not read as
    /// an external IRI would hand the dangling check a name the author
    /// never wrote, so it gaps and the value stays as authored.
    fn expand_bare(
        &mut self,
    ...
    fn note_unusable(&mut self, slot: &str, key: Option<String>, reason: String) {
        self.unusable_entries.push(UnusableCollectionEntry {
            record: self.current_holder.clone(),
            slot: slot.to_string(),
            key,
            reason,
        });
    ...
    /// Build the records of a collection authored in a dict spelling — an
    /// identifier-keyed mapping of records, or the compact SimpleDict form.
    /// Returns each built record's id and whether this call materialized
    /// it; entries that build nothing land in the unusable channel.
                ...
                _ => {
                    let Some(k) = key_str else {
                        self.note_unusable(
                            slot,
                            None,
                            "has a non-string key; quote the key to make it an id".to_string(),
                        );
                        continue;
                    };
                    if let Some(built) = self.build_simple_dict_entry(slot, candidates, k, record) {
                        ids.push(built);
                    }
                }
            }
    ...
    /// fact about which slot it fills, so the entry is unusable. Returns
    /// the built record's id and whether this call materialized it.
    fn build_simple_dict_entry(
        ...
        let Some(class) = winner else {
            self.note_unusable(
                slot,
                Some(key.to_string()),
                "is a compact entry, but no class of the range has a single open slot to fill"
                    .to_string(),
            );
            return None;
        };
    ...
    fn build_record(
        ...
        // nothing, silently turning references into literals.
        let resolved = self.resolved_slots(class_name);
        ...
        for (field_key, field_value) in map {
            ...
            let slot = resolved.get(field).map(|rs| &rs.definition);
            if slot.is_none() {
                self.undeclared_fields.push(UndeclaredField {
                    record: id.clone(),
            ...
            self.ingest_field(
                field,
                &ranges,
                slot.is_some_and(|s| crate::linkml_resolve::effective_cardinality(s).multivalued),
                &property,
                field_value,
                display,
                expand,
    ...
    fn ingest_field(
        ...
        let (class_targets, all_classes) = class_targets_of(self.schema, ranges);
        ...
        match value {
            // A mapping at a multivalued class-ranged slot is a collection
            // in a dict spelling — identifier-keyed records or compact
            // SimpleDict entries — read as it would be on the container.
            // One deliberate divergence: restating an already-materialized
            // record here is a citation (`held: false`), matching this
            // record level's inline and list spellings, where a container
            // collection holds every entry by role.
            serde_norway::Value::Mapping(map)
                if is_record_collection(multivalued, &class_targets, all_classes) =>
            {
                    ...
                    && !entries.is_empty()
                {
                    ...
                    self.note_expansion_gap(
                        &record,
                        slot,
                        expand_base.base_slot,
                        ExpansionGapKind::InlineRecord,
                    );
                }
                if entries.is_empty() && !map.is_empty() {
                    // Every entry was unusable: leave the authored value
                    // visible on the record, so a required-slot check or
                    // an absence claim cannot read the slot as unauthored.
                    push_slot_value(
                        slot_values,
                        slot,
                ...
                for (target, materialized) in entries {
            ...
            // unless the slot declares expansion, in which case a bare
            // scheme-less value is an anchor into the declared base's
            // namespace (the declaration only binds when no local record
            // of the range class can exist, so nothing is shadowed).
            serde_norway::Value::String(text) if all_classes => {
                let record = self.current_holder.clone().unwrap_or_default();
                let target = self
                    .expand_bare(expand, slot, &record, text, true)
                    .unwrap_or_else(|| text.clone());
                reference_to(target, false, references, slot_values);
            }
                ...
                if let Some(scalar) = scalar_value(other) {
                    let scalar = match scalar {
                        ScalarValue::String(bare) => {
                            let record = self.current_holder.clone().unwrap_or_default();
                            match self.expand_bare(expand, slot, &record, &bare, false) {
                                Some(expanded) => ScalarValue::String(expanded),
                                None => ScalarValue::String(bare),
                            }
                        }
                        scalar => scalar,
                    };
...
mod tests {
    ...
    /// At an expansion-declared class-ranged slot, a bare scheme-less
    /// value expands into the record's declared base and reads as an
    /// external reference into that namespace — never as an in-dataset
    /// reference — while CURIEs and absolute IRIs stay as authored.
    #[test]
    fn bare_values_at_a_class_ranged_anchor_slot_expand_to_external_references() {
        let schema: SchemaDefinition = serde_norway::from_str(
            "id: https://example.org/cqa\nname: cqa\ndefault_range: string\nprefixes:\n  cat: \
             https://example.org/catalog/\nclasses:\n  Benchmark:\n    tree_root: true\n    \
             slots: [id, questions]\n  CompetencyQuestionAnswer:\n    slots: [id, target_schema, \
             expected_anchors]\n  DomainRecord:\n    slots: [id]\nslots:\n  id: {identifier: \
             true}\n  target_schema: {range: uri}\n  questions: {range: CompetencyQuestionAnswer, \
             multivalued: true}\n  expected_anchors:\n    range: DomainRecord\n    multivalued: \
             true\n    annotations:\n      expand_against: target_schema\n",
        )
        .expect("schema");
        ...
        let external: Vec<&str> = q1
            ...
            .collect();
        assert!(
            external.contains(&"https://ns.example/wine/cabernet"),
            "the bare anchor expands into the declared namespace; got: {external:?}"
        );
        assert!(
            external.contains(&"cat:aws") && external.contains(&"https://abs.example/x"),
            "spelled anchors stay as authored; got: {external:?}"
        );
        assert!(
            set.expansion_gaps.is_empty(),
            "got: {:?}",
    ...
    /// At a class-ranged anchor slot with no usable base, only a bare
    /// scheme-less value is a gap: fully spelled anchors are the
    /// supported out-of-namespace authoring and owe nothing.
    #[test]
    fn class_ranged_anchors_gap_only_on_bare_values() {
        let schema: SchemaDefinition = serde_norway::from_str(
            "id: https://example.org/cqa\nname: cqa\ndefault_range: string\nprefixes:\n  cat: \
             https://example.org/catalog/\nclasses:\n  Benchmark:\n    tree_root: true\n    \
             slots: [id, questions]\n  CompetencyQuestionAnswer:\n    slots: [id, target_schema, \
        ...
        assert_eq!(
            set.expansion_gaps
                .iter()
                .map(|g| g.record.as_str())
                .collect::<Vec<_>>(),
            vec!["q_bare"],
            "the bare anchor gaps; the spelled ones owe no base"
        );
    }

    /// A base that does not form an absolute IRI cannot make a
    /// class-ranged anchor external, so the value stays as authored and
    /// the gap says why — never a fabricated in-dataset reference.
    #[test]
    fn a_relative_base_gaps_instead_of_minting_an_internal_reference() {
        let schema: SchemaDefinition = serde_norway::from_str(
            "id: https://example.org/cqa\nname: cqa\ndefault_range: string\nclasses:\n  \
             Benchmark:\n    tree_root: true\n    slots: [id, questions]\n  \
        ...
        assert_eq!(
            ...
            vec![ExpansionGapKind::UnusableBase(
                "a base that does not form an absolute IRI"
            )],
            "the relative base is the reported defect"
        );
        let q1 = set.instances.iter().find(|i| i.id == "q1").expect("q1");
        assert!(
            q1.slot_values
                ...
                .flat_map(|sv| &sv.values)
                .any(
                    |v| matches!(v, InstanceValue::Reference { target, .. } if target == "cabernet")
                ),
            "the value stays as authored, not concatenated into a phantom name"
        );
    }

    /// An inline record at a bound class-ranged slot is one finding per
    /// record, whatever the authoring spelling; a mapping at a bound
    /// scalar slot is a kind mismatch, not an inline record, and gets no
    /// expansion finding at all.
    #[test]
    fn inline_records_at_bound_slots_are_counted_per_record() {
        let schema: SchemaDefinition = serde_norway::from_str(
            "id: https://example.org/cqa\nname: cqa\ndefault_range: string\nclasses:\n  \
             Benchmark:\n    tree_root: true\n    slots: [id, questions]\n  \
             CompetencyQuestionAnswer:\n    slots: [id, target_schema, expected_anchors, \
        ...
        .expect("data");
        let set = InstanceSet::from_linkml_data(&schema, &data);
        let inline: Vec<&str> = set
            .expansion_gaps
            .iter()
            .filter(|g| g.kind == ExpansionGapKind::InlineRecord)
            .map(|g| g.record.as_str())
            .collect();
        assert_eq!(
            inline,
            vec!["q_list"],
            "one finding per claiming record and slot, whatever the spelling; got: {:?}",
            set.expansion_gaps
        );
        assert!(
            !set.expansion_gaps.iter().any(|g| g.record == "q_scalar"),
            "a mapping at a scalar slot is a kind mismatch, not an expansion finding; got: {:?}",
            set.expansion_gaps
        );

        let dict: serde_norway::Value = serde_norway::from_str(
            "id: b1\nquestions:\n  - id: q_dict\n    target_schema: https://ns.example/\n    \
             expected_anchors:\n      a: {}\n      b: {}\n",
        )
        .expect("data");
        let set = InstanceSet::from_linkml_data(&schema, &dict);
        assert_eq!(
            set.expansion_gaps
                .iter()
                .map(|g| (g.record.as_str(), g.kind.clone()))
                .collect::<Vec<_>>(),
            vec![("q_dict", ExpansionGapKind::InlineRecord)],
            "the dict spelling is the same one finding; got: {:?}",
            set.expansion_gaps
        );
    }

    /// A bound class-ranged slot carried by the `tree_root` itself reads
    /// exactly as it would on a nested record: bare values expand into
    /// external references, and an unusable base gaps against the root
    /// record.
    #[test]
    fn root_carried_anchor_slots_expand_like_nested_ones() {
        let schema: SchemaDefinition = serde_norway::from_str(
            "id: https://example.org/cqa\nname: cqa\ndefault_range: string\nclasses:\n  \
             Benchmark:\n    tree_root: true\n    slots: [id, target_schema, expected_anchors]\n  \
             DomainRecord:\n    slots: [id]\nslots:\n  id: {identifier: true}\n  target_schema: \
        ...
        .expect("data");
        let set = InstanceSet::from_linkml_data(&schema, &data);
        let root = set.instances.iter().find(|i| i.id == "b1").expect("root");
        assert!(
            root.references
                .iter()
                .any(|r| r.target == "https://ns.example/cabernet" && r.external),
            "the root's bare anchor expands to an external reference; got: {:?}",
            root.references
        );

        let missing: serde_norway::Value =
            serde_norway::from_str("id: b1\nexpected_anchors: [cabernet]\n").expect("data");
        let set = InstanceSet::from_linkml_data(&schema, &missing);
        assert_eq!(
            set.expansion_gaps
                .iter()
                .map(|g| (g.record.as_str(), g.kind.clone()))
    ...
    /// The root parity covers every value shape: a single-valued bound
    /// slot expands its bare string, and the dict spelling of inline
    /// records is one finding, exactly as on a nested record.
    #[test]
    fn root_parity_covers_single_values_and_dict_spellings() {
        let schema: SchemaDefinition = serde_norway::from_str(
            "id: https://example.org/cqa\nname: cqa\ndefault_range: string\nclasses:\n  \
             Benchmark:\n    tree_root: true\n    slots: [id, target_schema, primary_anchor, \
             expected_anchors]\n  DomainRecord:\n    slots: [id]\nslots:\n  id: {identifier: \
             true}\n  target_schema: {range: uri}\n  primary_anchor:\n    range: DomainRecord\n    \
             annotations:\n      expand_against: target_schema\n  expected_anchors:\n    range: \
             DomainRecord\n    multivalued: true\n    annotations:\n      expand_against: \
        ...
        assert!(
            root.references
                .iter()
                .any(|r| r.target == "https://ns.example/cabernet"),
            "the single-valued bare anchor expands; got: {:?}",
            root.references
        );
        assert_eq!(
            set.expansion_gaps
                .iter()
                .map(|g| (g.slot.as_str(), g.kind.clone()))
                .collect::<Vec<_>>(),
...
";
    ...
    fn an_inlined_object_at_a_single_class_range_is_built() {
        ...
        // Un-wrapped, the mapping is a dict-spelled collection whose key
        // names a field of the range — the same refusal and advice the
        // container gives, at any depth.
        let set = union_set("searches:\n  - {id: ls1, hasInput: {id: q9}}\n");
        assert!(
            !set.instances.iter().any(|i| i.id == "q9"),
            "no record builds from a field-named key"
        );
        assert!(
            set.unusable_collection_entries
                .iter()
                .any(|u| u.slot == "hasInput"
                    && u.reason.contains("wrap records in a list")
                    && u.record.as_deref() == Some("ls1")),
            "the entry is reported with the fix, naming its holding record; got: {:?}",
            set.unusable_collection_entries
        );
    }
  ...
  Sink:
    attributes:
      ...
      basin:
        range: string
";
        let schema: SchemaDefinition = serde_norway::from_str(NESTED_DICT_SCHEMA).expect("schema");
        let data: serde_norway::Value = serde_norway::from_str(
            "\
id: est
buildings:
...
",
        ...
        for (id, class) in [
            ...
            let entry = set
                ...
                .iter()
                .find(|i| i.id == id)
                .unwrap_or_else(|| panic!("{id} loads from the nested dict; got: {ids:?}"));
            assert_eq!(entry.types, vec![class], "{id} builds as its own member");
        }
        assert!(
            !set.instances.iter().any(|i| i.id.starts_with("Room-")),
            "no phantom record is minted from the dict itself; got: {ids:?}"
        );
        assert!(
            set.undeclared_fields.is_empty() && set.unusable_collection_entries.is_empty(),
            "every entry is read as data; got: {:?} / {:?}",
            set.undeclared_fields,
            set.unusable_collection_entries
        );
        let b1 = set.instances.iter().find(|i| i.id == "b1").expect("b1");
        for target in ["r1", "r2", "f1", "f2"] {
            assert!(
                b1.slot_values.iter().any(|sv| sv.values.iter().any(
                    |v| matches!(v, InstanceValue::Reference { target: t, .. } if t == target)
                )),
                "b1 holds an edge to {target}; got: {:?}",
                b1.slot_values
            );
        }
        assert!(
            b1.slot_values.iter().all(|sv| sv
                .values
                .iter()
                .all(|v| !matches!(v, InstanceValue::Unexpected(_)))),
            "a collection that loads leaves no unexpected-value residue; got: {:?}",
            b1.slot_values
        );

        // Cardinality decides multivalued-ness the same way it does
        // everywhere else: `maximum_cardinality: 3` admits the dict
        // spelling without a `multivalued` flag, and `maximum_cardinality:
        // 1` keeps a flagged slot single-valued, so its mapping is one
        // inline record, never a dict.
        let data: serde_norway::Value = serde_norway::from_str(
            "\
id: est
buildings:
...
",
        ...
        for id in ["n1", "p1"] {
            assert!(
                set.instances.iter().any(|i| i.id == id),
                "{id} loads under cardinality-spelled arity; got: {ids:?}"
            );
        }
        assert!(
            set.unusable_collection_entries.is_empty(),
            "neither spelling is refused; got: {:?}",
            set.unusable_collection_entries
        );

        // A dict whose every entry is unusable still marks the slot as
        // authored, so a required-slot check cannot call it absent.
        let data: serde_norway::Value =
        ...
        assert!(
            set.unusable_collection_entries
                .iter()
                .any(|u| u.record.as_deref() == Some("b3") && u.slot == "rooms"),
            "the finding names its holding record; got: {:?}",
            set.unusable_collection_entries
        );
        let b3 = set.instances.iter().find(|i| i.id == "b3").expect("b3");
        assert!(
            b3.slot_values.iter().any(|sv| sv.slot == "rooms"
                && sv
                    .values
                    .iter()
