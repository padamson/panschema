...
impl RustWriter {
    ...
    pub fn render_into<W: Write>(&self, out: &mut W, schema: &SchemaDefinition) -> fmt::Result {
        ...
        render_header(out, schema, self.time_crate)?;
...
fn render_header<W: Write>(out: &mut W, schema: &SchemaDefinition, time: TimeCrate) -> fmt::Result {
    ...
    writeln!(
        out,
        "// Requires: serde = {{ version = \"1\", features = [\"derive\"] }}, {time_dep}"
    )?;
