...
impl RustWriter {
    ...
    pub fn render_into<W: Write>(&self, out: &mut W, schema: &SchemaDefinition) -> fmt::Result {
        ...
        // The body renders first: whether any designated enum was
        // emitted decides the header's dependency line.
        let mut body = String::new();
        let mut designated = false;

        for (name, def) in &schema.enums {
            render_enum(&mut body, name, def)?;
        }
        ...
        for (name, def) in &schema.classes {
            if roles.get(name) == Some(&ClassRole::Trait) {
                render_trait(&mut body, name, def, schema, &roles)?;
            }
        }
        for (name, def) in &schema.classes {
            if roles.get(name) == Some(&ClassRole::Struct) {
                render_class(&mut body, name, def, &ctx, &mut any_of_enums)?;
            }
        }
        let designator_by_class = designators_by_class(schema);
        for name in schema.classes.keys() {
            if roles.get(name) == Some(&ClassRole::Trait) {
                designated |= render_kind_enum(
                    &mut body,
                    name,
                    schema,
                    &roles,
        ...
        render_header(out, schema, self.time_crate, designated)?;
        out.write_str(&body)
    }
}
...
fn render_header<W: Write>(
    ...
    // Name the consumer's Cargo.toml requirements, feature flags included
    // — a jiff module without jiff's `serde` feature fails to compile with
    // missing-impl errors that don't say why.
    let time_dep = match time {
        TimeCrate::Chrono => "chrono = { version = \"0.4\", features = [\"serde\"] }",
        TimeCrate::Jiff => "jiff = { version = \"0.2\", features = [\"serde\"] }",
    };
    // A designated enum's generated deserializer buffers through
    // `serde_json::Value` to peek the designator, so those modules need
    // serde_json too.
    let json_dep = if designated {
        ", serde_json = \"1\""
    } else {
        ""
    };
    writeln!(
        out,
        "// Requires: serde = {{ version = \"1\", features = [\"derive\"] }}{json_dep}, {time_dep}"
    )?;
...
fn render_kind_enum<W: Write>(
    ...
    let eq_hash_ok = eq_hash_support.get(name).copied().unwrap_or(false);
    let designator = designators.get(name).cloned().flatten();
    write!(
        out,
    ...
    writeln!(
        out,
        "#[derive({})]",
        enum_derive_line(eq_hash_ok, designator.is_none())
    )?;
    out.write_str("#[serde(untagged)]\n")?;
    out.write_str("#[non_exhaustive]\n")?;
    let enum_name = type_ident(&format!("{name}Kind")).into_owned();
    ...
    out.write_str("}\n\n")?;
    let Some(key) = designator else {
        return Ok(false);
    };
    // Only `is_a` descendants dispatch — the loader's designator rule
    // never walks mixins — and each answers to its own spellings alone:
    // the single-class rule types a record as exactly the class named.
    // A mixin-only variant stays reachable through the shape fallback.
    let own_names: Vec<Option<String>> = descendants
        .iter()
        .map(|desc| {
            crate::linkml_resolve::class_satisfies(schema, desc, name).then(|| desc.clone())
        })
        .collect();
    let mut contributions: Vec<(String, usize)> = Vec::new();
    for (position, desc) in descendants.iter().enumerate() {
...
fn render_any_of_enum<W: Write>(
    ...
    let shared = {
        let mut per_member = members
            .iter()
            .map(|member| designators.get(member).cloned().flatten());
        match per_member.next().flatten() {
            Some(first) if per_member.all(|d| d.as_deref() == Some(first.as_str())) => Some(first),
            _ => None,
        }
    };
    ...
    writeln!(
        out,
        "#[derive({})]",
        enum_derive_line(eq_hash_ok, shared.is_none())
    )?;
    out.write_str("#[serde(untagged)]\n")?;
    out.write_str("#[non_exhaustive]\n")?;
    writeln!(out, "pub enum {} {{", type_ident(name))?;
...
/// both enum emitters consult it, and the full slot resolution is too
/// costly to repeat per union member.
fn designators_by_class(schema: &SchemaDefinition) -> BTreeMap<String, Option<String>> {
    schema
        .classes
        .iter()
        .map(|(name, class)| {
            (
                name.clone(),
                crate::linkml_resolve::designator_slot_of(class, schema),
            )
        })
        .collect()
}
...
/// `Eq + Hash` toggle on; `Deserialize` drops out when the enum gets a
/// generated designator-dispatching impl instead of the derive.
fn enum_derive_line(eq_hash_ok: bool, derive_deserialize: bool) -> &'static str {
    match (eq_hash_ok, derive_deserialize) {
        (true, true) => "Debug, Clone, PartialEq, Eq, Hash, serde::Serialize, serde::Deserialize",
        (false, true) => "Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize",
        (true, false) => "Debug, Clone, PartialEq, Eq, Hash, serde::Serialize",
        (false, false) => "Debug, Clone, PartialEq, serde::Serialize",
    }
}

/// One variant of a designated enum, ready for dispatch emission: the
...
/// What the loader does with a designator value the table cannot
/// resolve — mirrored per enum kind, so a call site names the loader
/// rule it follows.
enum Unresolvable {
    /// A union refuses the record rather than guessing.
    Errors,
    /// A single-class range keeps the declared class standing, which
    /// in generated code is the shape fallback.
    FallsBackToShape,
}

/// Distribute designator spellings over an enum's variants, mirroring
/// the loader's precedence (see `member_by_family` in instances.rs —
/// the two encode one rule, evaluated at run and generate time): each
/// variant's own principal-class name wins outright, exactly as the
/// loader's members-first tier does; every other spelling belongs to
/// the single variant claiming it, and a spelling several variants
/// claim names no one of them — it falls to the unresolvable arm.
fn assign_spellings(
    variants: &mut [DesignatedVariant],
    ...
    for (spelling, owner) in contributions {
        claimed.entry(spelling).or_default().insert(owner);
    }
    let mut names: std::collections::BTreeSet<&str> = std::collections::BTreeSet::new();
    for (position, own) in own_names.iter().enumerate() {
        if let Some(name) = own {
            variants[position].spellings.push(name.clone());
            names.insert(name);
        }
    }
    for (spelling, owners) in claimed {
        if names.contains(spelling.as_str()) {
            continue;
        }
        if owners.len() == 1 {
            let owner = *owners.iter().next().expect("one owner");
            variants[owner].spellings.push(spelling);
        }
    }
}

/// The generated `Deserialize` for a designated enum: the designator
/// value is peeked — never consumed, so the variant struct still sees
/// it — and resolved through the compiled table; an absent designator
/// falls back to shape, first fit, the untagged rule; a present but
...
fn render_designated_deserialize<W: Write>(
    ...
    writeln!(out, "        match value.get(\"{}\") {{", escape_str(key))?;
    writeln!(
        out,
        "            Some(serde_json::Value::String(named)) => match named.as_str() {{"
    )?;
    ...
    match unresolvable {
        Unresolvable::Errors => {
            ...
            writeln!(
                out,
                "                        \"designator `{{other}}` names no class of this union\""
            )?;
            writeln!(out, "                    )));")?;
            writeln!(out, "                }}")?;
        }
        Unresolvable::FallsBackToShape => {
            writeln!(
                out,
                "                // An unresolvable designator falls back to shape,"
            )?;
            writeln!(
                out,
                "                // as the loader keeps the declared class."
            )?;
            writeln!(out, "                _ => {{}}")?;
        }
    }
    ...
    match unresolvable {
        Unresolvable::Errors => {
            writeln!(
                out,
                "            // Present but not a string: it can name no class,"
            )?;
            writeln!(
                out,
                "            // and a union never guesses around a designator."
            )?;
            writeln!(out, "            Some(_) => {{")?;
            writeln!(out, "                return Err(serde::de::Error::custom(")?;
            writeln!(
                out,
                "                    \"the designator must be a string naming a class\","
            )?;
            writeln!(out, "                ));")?;
            writeln!(out, "            }}")?;
        }
    ...
    writeln!(
        out,
        "        Err(serde::de::Error::custom(\"no member of `{enum_name}` fits the value\"))"
    )?;
    writeln!(out, "    }}")?;
    writeln!(out, "}}")?;
    out.write_str("\n")
...
mod tests {
    ...
    fn render_kind_enum_lists_concrete_descendants() {
        ...
        // answering to its own spellings only — and falls back to shape
        // when the designator is absent or names nothing, as the loader
        // keeps the declared class there.
        ...
        assert!(
            ...
            "the derive would conflict with the generated impl; got: {out}"
        );
        assert!(
            out.contains("falls back to shape"),
            "an unresolvable value keeps the shape fallback; got: {out}"
        );

        // A mixin-only descendant is a variant — the enum's shape — but
        // never a dispatch target: the loader's designator rule walks
        // `is_a` alone, so `Robot` stays reachable only through shape.
        let mut robot = ClassDefinition::new("Robot");
        robot.mixins = vec!["Animal".to_string()];
        schema.classes.insert("Robot".to_string(), robot);
        let roles = compute_class_roles(&schema);
        let mut out = String::new();
        render_kind_enum(
            &mut out,
        ...
        .unwrap();
        assert!(
            out.contains("Robot(Box<Robot>)"),
            "the mixin descendant stays a variant; got: {out}"
        );
        assert!(
            !out.contains("\"Robot\""),
            "a designator can never name a mixin-only descendant; got: {out}"
        );
        assert!(
            out.contains("value.get(\"kind\")"),
            "the impl peeks the class's own designator slot; got: {out}"
        );
    }

    /// One full render: each construct comes out of its own emission
    /// loop, and a designated enum — kind or union, each alone — puts
    /// `serde_json` in the header's requirements.
    #[test]
    fn render_emits_constructs_by_role_and_flags_designated_modules() {
        let mut schema = SchemaDefinition::new("s");
        let mut animal = ClassDefinition::new("Animal");
        animal.r#abstract = true;
        let mut kind = SlotDefinition::new("kind");
        kind.designates_type = true;
        animal.attributes.insert("kind".to_string(), kind.clone());
        ...
        let mut schema = SchemaDefinition::new("s");
        for member in ["Lamp", "Sink"] {
            let mut class = ClassDefinition::new(member);
            class.attributes.insert("kind".to_string(), kind.clone());
            schema.classes.insert(member.to_string(), class);
        }
        let mut holder = ClassDefinition::new("Holder");
        let mut fixture = SlotDefinition::new("fixture");
        fixture.any_of = vec![slot_with_range("", "Lamp"), slot_with_range("", "Sink")];
        holder.attributes.insert("fixture".to_string(), fixture);
        schema.classes.insert("Holder".to_string(), holder);
        let out = RustWriter::new().render(&schema);
        assert!(
        ...
        assert!(
            out.contains("serde_json = \"1\""),
            "a designated union alone requires serde_json; got: {out}"
        );
    }
    ...
    #[test]
    fn a_designated_union_gets_a_designator_aware_deserializer() {
        // Every member designates through `designator`; Whisk is Sink's
        // subclass, and Basin is an unrelated class whose name must not
        // enter the table.
        let mut schema = SchemaDefinition::new("s");
        schema.default_prefix = Some("x".to_string());
        schema
            .prefixes
            .insert("x".to_string(), "https://example.org/x/".to_string());
        for member in ["Lamp", "Sink"] {
            let mut class = ClassDefinition::new(member);
            let mut designator = SlotDefinition::new("designator");
            designator.designates_type = true;
            class
                .attributes
                .insert("designator".to_string(), designator);
            schema.classes.insert(member.to_string(), class);
        }
        let mut whisk = ClassDefinition::new("Whisk");
        whisk.is_a = Some("Sink".to_string());
        schema.classes.insert("Whisk".to_string(), whisk);
        schema
            .classes
            .insert("Basin".to_string(), ClassDefinition::new("Basin"));
        ...
        assert!(
            !out.contains("Deserialize)]") && !out.contains(", Deserialize"),
            "the derive would conflict with the generated impl; got: {out}"
        );
        assert!(
            out.contains("#[serde(untagged)]"),
            "serialization stays transparent; got: {out}"
        );
        for spelling in [
            "\"Sink\"",
            "\"Whisk\"",
            "\"x:Sink\"",
            "\"https://example.org/x/Sink\"",
        ] {
            assert!(
                out.contains(spelling),
                "every spelling of a member or its subclass dispatches: {spelling}; got: {out}"
            );
        }
        assert!(
            !out.contains("\"Basin\""),
            "an unrelated class never enters the table; got: {out}"
        );
        assert!(
        ...
        schema.classes.get_mut("Sink").unwrap().attributes.clear();
        let mut out = String::new();
        render_any_of_enum(
            &mut out,
            "Fixture",
        ...
        .unwrap();
        assert!(
            out.contains("serde::Deserialize") && !out.contains("impl<'de>"),
            "a union without a shared designator derives its Deserialize; got: {out}"
        );
    }
