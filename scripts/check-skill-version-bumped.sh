...
# everything since the merge base with that ref — the aggregate that
# actually lands — because the hook is per-clone and `--no-verify` skips
# it. The two can disagree on one commit (a bump in one commit and an
# unbumped edit in the next passes CI and fails the hook on the second), and
# that is the intent: the hook keeps history honest, CI keeps what lands
# honest.
#
# One fixed-shape read, and the Rust test on the same frontmatter requires
# the same shape: the version is the double-quoted string on the `version:`
# line under `metadata:`. A bare number or single quotes read as no version
# here and fail the test there, so the two checks cannot disagree about one
# file. (Three parsers of one frontmatter disagreed on exactly those cases
