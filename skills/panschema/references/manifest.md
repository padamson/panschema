...
| `verify_absences` | **Not an output** — binds a slot as a stated absence claim, verified against the `resolve_against` siblings: `verify_absences = { slot = "unconnected_anchors", via = "connecting_class" }`. A record listing two or more anchors under `slot` (references or IRI scalars) claims no single sibling record references them all; `via` (optional) names a slot whose value — a class IRI, CURIE or absolute — narrows the claim to joining records of that class. The sibling's dataset container is exempt by identity (holding is not joining). A claim the check can't evaluate — anchors collapsing to one IRI, an anchor no sibling mints, a `via` naming no sibling class — is reported uncheckable, never as holding. Contradicted and uncheckable claims warn; `--strict` fails. Needs `resolve_against`; binding a slot no class carries is an error. The binding lives here so the data model carries no tool annotations |
