...
| `pattern` | ● | ● | ● | ○ | ○ | ✗ | "Pattern" row (truncated + tooltip); not enforced in RDF/Rust; Postgres `CHECK (col ~ 'pattern')` is [feature 24 slice 2](features/24-postgres-ddl-writer.md), not yet built |
...
| `minimum_value` `maximum_value` | ● | ● | — | ○ | — | ✗ | numeric value bounds: `≥`/`≤` card badge (feature 14 slice 2); RDF `owl:withRestrictions` facet deferred (slice 2b); Postgres `CHECK` bounds are [feature 24 slice 2](features/24-postgres-ddl-writer.md), not yet built |
