...
#### Slice 4b-cont: `value_presence` + `any_of` conditions (2026-07-24)

- [x] A `value_presence` slot condition projects into the same bounds the shape already carries: `PRESENT` → `sh:minCount 1` ("at least one value"), `ABSENT` → `sh:maxCount 0` ("no values") (`value_presence_present_projects_to_min_count_one`, `value_presence_absent_projects_to_max_count_zero`).
- [x] A slot-level `any_of` (alternative values on one slot) projects as `sh:or` over per-alternative constraint shapes on the property shape, each carrying its alternative's `sh:hasValue`/bounds — with the slot's declared range flowing into every alternative so `equals_number` literals stay correctly typed (`slot_any_of_projects_to_sh_or_of_alternatives`).
- [x] A rule-level `any_of` (alternative whole condition sets) projects as `sh:or` over recursively-emitted alternative condition-set shapes; a condition side that is *only* `any_of` (empty `slot_conditions`) is now expressible and no longer skipped, while a side with neither stays skipped (`rule_level_any_of_projects_alternative_condition_sets`, `a_condition_with_neither_slots_nor_alternatives_still_skips`). Slots referenced inside alternatives are validated recursively before emission, preserving the never-fabricate-an-IRI rule.
