...
- **Rust types writer** (work in progress; see [docs/features/06-rust-codegen.md](docs/features/06-rust-codegen.md)). `[generate.<name>]` now accepts a `rust = "<path>"` key alongside `html`, and `panschema generate` fans out across every populated writer per schema. Slice 6.1 lands the fan-out plumbing and a `RustWriter` stub that emits the `@generated` header; subsequent slices fill in struct/enum/trait codegen for inheritance chains, mixins, `slot_usage` overrides, polymorphic `any_of` ranges, and LinkML descriptions as Rust doc-comments.
